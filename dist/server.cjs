...
var redisUrl = process.env.UPSTASH_REDIS_URL || process.env.REDIS_URL || "redis://localhost:6379";
...
// src/server/workers/syllabusWorker.ts
...
function setModelCooldown(model, durationMs = 18e4) {
...
async function generateWithModelFallback(ai, params) {
  ...
  for (const model of orderedModels) {
    ...
    try {
      ...
      const isQuotaExhausted = msg.includes("429") || msg.includes("RESOURCE_EXHAUSTED") || msg.includes("quota") || msg.includes("exceeded your current quota");
      const isHighDemand = msg.includes("503") || msg.includes("UNAVAILABLE") || msg.includes("high demand") || msg.includes("temporarily unavailable");
      if (isQuotaExhausted) {
        setModelCooldown(model, 18e4);
        console.info(`[Gemini API] ${model} quota reached. Cascading to next candidate model...`);
        continue;
      }
      if (isHighDemand) {
        setModelCooldown(model, 45e3);
...
async function generateStreamWithModelFallback(ai, params) {
  let lastError = null;
  const orderedModels = [
    ...CANDIDATE_MODELS.filter(isModelHealthy),
    ...CANDIDATE_MODELS.filter((m) => !isModelHealthy(m))
  ...
  for (const model of orderedModels) {
    ...
    try {
      ...
      lastError = err;
      const msg = String(err?.message || "");
      const isQuotaExhausted = msg.includes("429") || msg.includes("RESOURCE_EXHAUSTED") || msg.includes("quota");
...
function generateFallbackCurriculum(subjectGoal = "Mastery Path", level = "intermediate", language = "English", weeklyHours = 5) {
...
// src/server/workers/syllabusWorker.ts
var syllabusWorker = new import_bullmq.Worker(
...
async function generateTTS(text, language = "English") {
  try {
...
function extractModulesFromIncompleteJSON(text) {
  const modules = [];
...
var curriculumQueue = new import_bullmq2.Queue("curriculum-generation", { connection: redis_default });
...
app.get("/api/audio/:hash", async (req, res) => {
  ...
  try {
    const base64Audio = await redis_default.get(`tts:${hash}`);
    if (!base64Audio) {
...
app.post("/api/generate-lesson", async (req, res) => {
  ...
  try {
    ...
    const ai = getGemini();
    if (ai) {
      const studentContext = studentProfile ? `Student Name: ${studentProfile.name || "Learner"}, Current Mastery Level: ${studentProfile.level || level}, Weak Concepts to Address: ${(studentProfile.weakConcepts || []).join(", ") || "None specified"}, Preferred Style: ${studentProfile.style || style}` : `Learner Level: ${level}, Preferred Style: ${style}`;
      ...
      for await (const chunk of stream) {
        ...
        for (const mod of modules) {
          if (!processedModuleIds.has(mod.id)) {
            ...
            if (mod.speechScript) {
              const hash = import_crypto.default.createHash("sha256").update(mod.speechScript).digest("hex");
              const cached = await redis_default.get(`tts:${hash}`);
              if (!cached) {
                const audioBuffer = await generateTTS(mod.speechScript, language);
                if (audioBuffer) {
                  await redis_default.set(`tts:${hash}`, audioBuffer.toString("base64"));
...
`);
      ...
      try {
        ...
        delete metadata.curriculumModules;
...
app.post("/api/generate-curriculum", async (req, res) => {
  try {
    ...
    const hash = import_crypto.default.createHash("sha256").update(JSON.stringify(payload)).digest("hex");
    const cacheKey = `curriculum:${hash}`;
    const cachedResult = await redis_default.get(cacheKey);
    if (cachedResult) {
...
app.get("/api/curriculum-job/:id", async (req, res) => {
  try {
    ...
    if (state === "completed") {
      ...
      await redis_default.set(`curriculum:${id}`, JSON.stringify(curriculum), "EX", 86400);
...
app.get("/api/lessons", async (req, res) => {
  try {
    ...
    const lessons = dbLessons.map((dbLesson) => {
      const parsedModules = dbLesson.modules || {};
      return {
        id: dbLesson.id,
  ...
  try {
    ...
    const savedLesson = await prisma.lesson.create({
      data: {
        ...
        modules: rest
        // Store all extra fields like lesson plan, teacherAvatarUrl, level in the JSON blob
      }
    });
...
function generateFallbackPractice(topic = "Core Principles", level = "intermediate", language = "English", conceptToTest = "") {
  const isHindi = language.toLowerCase().includes("hindi") || language.toLowerCase().includes("hinglish");
...
function generateFallbackLesson(topic = "Ohm's Law & Electrical Circuits", level = "beginner", duration = 20, language = "English", teacherName = "Andrei Baranov") {
  const isHindi = language.toLowerCase().includes("hindi") || language.toLowerCase().includes("hinglish");
      ...
      curriculumModules: [
        {
          ...
          type: "explain",
          speechScript: isHindi ? "Namaste! Aaj hum React ka core concept samjhenge: UI state ka direct reflection hoti hai. Jab bhi state badalegi, React automatically UI ko refresh karega." : "Welcome! Today we will break down React state. In declarative UI, you describe what the interface should look like for a given state, and React handles the DOM updates.",
    ...
    <button onClick={() => setCount(prev => prev + 1)}>
      Clicks: {count}
        ...
        },
        {
          ...
          type: "demonstrate",
          speechScript: isHindi ? "Chaliye execution flow dekhte hain. Parent component data niche pass karta hai props ke zariye, aur events upar emit hote hain." : "Let us visualize the data flow. Data cascades downwards via props, while user interactions bubble upwards via callbacks.",
        ...
        },
        {
          ...
          type: "adapt",
          speechScript: isHindi ? "Ek bohot common mistake: setState call karne ke turant baad console.log me naya state nahi milta, kyunki React updates ko batch karta hai." : "Here is an area where many developers stumble: React state updates are asynchronous and batched. Inspecting state immediately after calling its setter still returns the previous value.",
...
setCount(prev => prev + 1);`,
  ...
  if (isPhysics || !isCode && (!topic || topic.toLowerCase().includes("ohm") || topic.toLowerCase().includes("circuit"))) {
    return {
      ...
      summary: `Deep dive into the fundamental relationship connecting Voltage, Current, and Resistance (V = I \xB7 R) with adaptive misconception detection.`,
      subjectCategory: "physics",
      estimatedTimeMinutes: duration,
      learningObjectives: [
        "Master the governing equation V = I \xB7 R",
      ...
      curriculumModules: [
        {
          ...
          type: "explain",
          speechScript: isHindi ? "Swagat hai dosto! Aaj hum physics ka ek sabse basic aur powerful law samjhenge: Ohm ka niyam. Voltage ek dhakka hai, jo electrons ko wire ke andar aage badhata hai." : "Welcome to EchoMind Physics! Today we will break down Ohm's Law. Think of Voltage as electrical pressure\u2014the force pushing electrons through a conductive pathway.",
        ...
        },
        {
          ...
          type: "demonstrate",
          speechScript: isHindi ? "Isko paani ke pipe se socho: Voltage paani ki tanki ki unchai hai. Current paani ki dhaar hai. Aur Resistance pipe ka patla hona hai." : "Let us use a vivid analogy: Imagine a water pipe. Voltage is the water pressure pump. Current is the volume of water flowing per second. Resistance is narrowing the pipe.",
        ...
        },
        {
          ...
          type: "adapt",
          speechScript: isHindi ? "Ab ek practical sawal lagate hain. Agar humare paas 12 Volt ki battery hai aur 4 Ohm ka resistor hai, toh circuit me kitna current daudega?" : "Now let us test our understanding with a real calculation. If you connect a 12 Volt power source across a 4 Ohm precision resistor, what is the resulting current?",
          ...
          visualContent: {
            ...
            details: [
              ...
              "Resulting heat dissipation: P = I\xB2R = (3)\xB2 \xD7 4 = 36 Watts"
            ],
          ...
          checkpointQuestion: {
            ...
            correctIndex: 1,
            commonMisconception: "Multiplying V \xD7 R instead of dividing V / R.",
            explanation: "Current is I = V / R = 12 / 4 = 3 Amperes. Multiplying 12 \xD7 4 gives 48, which is mathematically inverted."
          }
        }
      ],
      finalAssessment: [
    ...
    curriculumModules: [
      {
        ...
        type: "explain",
        speechScript: isHindi ? `Namaste! Aaj hum ${cleanTopic} ke mukhya siddhant samjhenge. Kisi bhi vishay ko samajhne ke liye uski neenv mazboot hona sabse zaroori hai.` : `Welcome! Today we will break down ${cleanTopic}. To master this subject, we first establish the foundational laws that govern how it operates.`,
      ...
      },
      {
        ...
        type: "demonstrate",
        speechScript: isHindi ? `Chaliye isko ek practical udaharan se dekhte hain. Jab hum iske prabhav ko observe karte hain, toh cause and effect ka sambandh saaf dikhta hai.` : `Let us ground this in a real-world demonstration. Notice how shifting the primary variable creates a direct, measurable cascade through the entire system.`,
      ...
      },
      {
        ...
        type: "adapt",
        speechScript: isHindi ? `Ab chaliye ek practical scenario solve karte hain. Jo concepts humne sikhe hain, unhe apply karke dekhte hain.` : `Now let us put theory into practice with a diagnostic challenge. We will apply the principles we covered to solve a realistic scenario.`,
...
function generateFallbackEvaluation(question, studentAnswer, concept, language) {
  ...
  if (question.includes("resistance increases") || concept.includes("Ohm") || concept.includes("current")) {
    if (answerLower.includes("increase") || answerLower.includes("badhega") || answerLower.includes("zyada")) {
      return {
        isCorrect: false,
        confidenceScore: 95,
        feedback: isHindi ? "Aapne kaha current badhega\u2014lekin rukiye! Yahan ek bohot common misconception hai jo bohot se students karte hain." : "You answered that current increases\u2014let us pause right there! This is one of the most classic misconceptions in electrical physics.",
        ...
        intuitiveAnalogy: isHindi ? "Ek pipe ke baare me socho: agar pipe ke andar kachra bhar jaye (resistance badh jaye), toh paani ka bahav (current) kam ho jayega, badhega nahi!" : "Think of a highway toll plaza: If more toll gates are closed (increased resistance), does traffic move faster or slower? It slows down! Resistance always opposes the flow.",
      ...
      return {
        isCorrect: true,
        confidenceScore: 98,
        feedback: isHindi ? "Bilkul sahi jawab! Shabaash. Resistance badhne se current kam hota hai kyunki dono inversely proportional hain." : "Spot on! Excellent scientific reasoning. Since Resistance directly opposes charge transport, increasing it under constant voltage decreases current.",
  ...
  return {
    isCorrect: true,
    confidenceScore: 85,
    feedback: isHindi ? "Aapne achha prayas kiya! Yeh concept samajhne ke liye bahut zaroori hai." : "Great response! You are engaging directly with the core mechanics of this principle.",
    misconceptionDetected: null,
